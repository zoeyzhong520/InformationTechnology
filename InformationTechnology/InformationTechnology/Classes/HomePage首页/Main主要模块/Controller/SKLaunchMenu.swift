...
class SKLaunchMenu: UIViewController,UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    private func createLaunchMenu() {
        let height = layout.itemSize.height*CGFloat(subViewTitles.count/2)
        launchMenu = UICollectionView(frame: CGRectMake(0, kStateBarHeight, kScreenWidth, height), collectionViewLayout: layout)
